import java.awt.*;
import java.awt.event.*;
import java.util.*;

public class MazeSolverBFS extends Frame {

    private static final int CELL = 50;
    private static final int SIDE = 650;

    private int[][] maze;
    private int rows, cols;

    private int[] start;
    private int[] end;

    private Label msgLabel;
    private final Button startBtn;
    private Button pauseBtn;
    private boolean isPaused = false;

    private boolean[][] visited;
    
    private java.util.List<int[]> path;

    private boolean showVisited = false;
    private boolean showFinalPath = false;

    public MazeSolverBFS(int[][] maze, int[] start, int[] end) {
        
        this.maze = maze;
        rows = this.maze.length;
        cols = this.maze[0].length;
        this.start = start;
        this.end = end;

        setLayout(new BorderLayout());

        Panel btnpanel = new Panel();
        btnpanel.setLayout(new FlowLayout());

        startBtn = new Button("Start BFS Traversal");
        pauseBtn = new Button("Pause");

        btnpanel.add(startBtn);
        btnpanel.add(pauseBtn);

        msgLabel = new Label("Click on Start to begin", Label.CENTER);
        msgLabel.setFont(new Font("Arial", Font.BOLD, 18));

        add(btnpanel, BorderLayout.NORTH);
        add(msgLabel, BorderLayout.SOUTH);

        startBtn.addActionListener(e -> {
            
            msgLabel.setText("BFS Traversing...");
            new Thread(this::startVisualization).start();
        });

        pauseBtn.addActionListener(e -> {
            
            isPaused = !isPaused;
            if (isPaused) {
                
                pauseBtn.setLabel("Resume");
                msgLabel.setText("Visualization paused");
            } 
            else {
               
                pauseBtn.setLabel("Pause");
                msgLabel.setText("BFS Traversing");
            }
        });

        setTitle("Maze Solver Visualization - BFS");
        setSize(SIDE, SIDE);
        setVisible(true);

        addWindowListener(new WindowAdapter() {
            @Override 
            public void windowClosing(WindowEvent we) {
                System.exit(0);
            }
        });
    }

    @Override
    public void paint(Graphics g) {
        int gridW = cols*CELL;
        int gridH = rows*CELL;
        int xPos = (SIDE - gridW)/2;
        int yPos = (SIDE - gridH)/2;

        // initial maze
        for (int r=0; r<rows; r++) {
            for (int c=0; c<cols; c++) {
                
                if (maze[r][c] == 1) {
                    g.setColor(new Color(54, 69, 79));
                }
                else {
                    g.setColor(Color.WHITE);
                }
                g.fillRect(xPos + c*CELL, yPos + r*CELL, CELL, CELL);
                
                g.setColor(Color.GRAY);
                g.drawRect(xPos + c*CELL, yPos + r*CELL, CELL, CELL);
            }
        }

        // traversal
        if (showVisited && visited != null) {

            g.setColor(new Color(135, 206, 250));
            
            for (int r = 0; r < rows; r++) {
                for (int c = 0; c < cols; c++) {
            
                    if (visited[r][c] && !(r == start[0] && c == start[1]) && !(r == end[0] && c == end[1])) {
                        
                        g.fillRect(xPos + c*CELL, yPos + r*CELL, CELL, CELL);
                        
                        g.setColor(Color.GRAY);
                        g.drawRect(xPos + c*CELL, yPos + r*CELL, CELL, CELL);
                        
                        g.setColor(new Color(135, 206, 250));
                    }
                }
            }
        }

        // final path
        if (showFinalPath && path != null) {
            
            g.setColor(new Color(34, 240, 95));
            
            for (int[] p : path) {
            
                int r = p[0], c = p[1];
                if (!(r == start[0] && c == start[1]) && !(r == end[0] && c == end[1])) {
            
                    g.fillRect(xPos + c*CELL, yPos + r*CELL, CELL, CELL);
                    
                    g.setColor(Color.GRAY);
                    g.drawRect(xPos + c*CELL, yPos + r*CELL, CELL, CELL);
                    
                    g.setColor(new Color(34, 240, 95));
                }
            }
        }

        g.setColor(new Color(0, 128, 255));
        g.fillRect(xPos + start[1]*CELL, yPos + start[0]*CELL, CELL, CELL);

        g.setColor(Color.RED);
        g.fillRect(xPos + end[1]*CELL, yPos + end[0]*CELL, CELL, CELL);
    }

    private void delay(long ms) {
        try {
            Thread.sleep(ms);
            while (isPaused) {
                Thread.sleep(100);
            }    
        } 
        catch (InterruptedException ie) {
            Thread.currentThread().interrupt();
        }
    }

    private void startVisualization() {
        
        visited = new boolean[rows][cols];
        path = new ArrayList<>();
        
        showVisited = true;
        showFinalPath = false;

        msgLabel.setText("BFS started");

        boolean found = bfs();

        if (found) {
            
            msgLabel.setText("BFS complete: Path found.");
            delay(2000);
            
            showVisited = false;
            showFinalPath = true;
            
            repaint();
            msgLabel.setText("Final path highlighted");
        } else {
            
            msgLabel.setText("BFS complete: No path found");
            delay(2000);
            
            showVisited = true;
            showFinalPath = false;

            repaint();
            msgLabel.setText("No path exists for this maze");
        }
    }

    private boolean bfs() {
        
        Queue<int[]> queue = new LinkedList<>();
        Map<String, int[]> parent = new HashMap<>();

        queue.add(start);
        
        visited[start[0]][start[1]] = true;

        int[][] dirs = {{1,0},{-1,0},{0,1},{0,-1}};

        while (!queue.isEmpty()) {
        
            int[] cur = queue.poll();
            int r = cur[0], c = cur[1];

            repaint();
            delay(750);

            if (r == end[0] && c == end[1]) {
                
                buildPath(parent, end);
                return true;
            }

            for (int[] d : dirs) {
                
                int nr = r + d[0], nc = c + d[1];
                if (nr >= 0 && nr < rows && nc >= 0 && nc < cols && maze[nr][nc] == 0 && !visited[nr][nc]) {
                
                    queue.add(new int[]{nr, nc});
                    visited[nr][nc] = true;
                    parent.put(nr + "," + nc, cur);
                }
            }
        }
        return false;
    }

    private void buildPath(Map<String, int[]> parent, int[] end) {
        
        ArrayList<int[]> revPath = new ArrayList<>();
        
        int[] cur = end;
        
        while (cur != null) {
        
            revPath.add(cur);
            cur = parent.get(cur[0] + "," + cur[1]);
        }
        
        Collections.reverse(revPath);
        path = revPath;
    }

    public static void main(String[] args) {

        Scanner sc = new Scanner(System.in);

        System.out.print("Enter rows: ");
        int r = sc.nextInt();

        System.out.print("Enter cols: ");
        int c = sc.nextInt();

        int[][] mazeInput = new int[r][c];
        
        int[] start = null;
        int[] end = null;
        int start_end = 0; // count of -1

        System.out.println("Enter maze (0=open, 1=wall, -1=start/end):");
        for (int i = 0; i < r; i++) {
            for (int j = 0; j < c; j++) {
        
                int val = sc.nextInt();
        
                if (val == -1) {
                    start_end++;
        
                    if (start_end == 1) {
                        start = new int[]{i, j};
                    } 
                    else if (start_end == 2) {
                        end = new int[]{i, j};
                    } 
                    else {
                        System.out.println("Error: More than 2 -1 values entered.");
                        return;
                    }
                    
                    mazeInput[i][j] = 0; 
                } 
                else {
                    mazeInput[i][j] = val;
                }
            }
        }

        if (start_end < 2) {
            System.out.println("Error: enter exactly 2 -1 values for start and end." + start_end);
            return;
        }

        new MazeSolverBFS(mazeInput, start, end);
    }
}
